...
/// Корневой элемент DN - Результаты верификации (для экспорта)

[XmlRoot("DN")]
public class GfExportDto
{
    [XmlElement("ZGLV")]
    public GfExportHeader? Header { get; set; }

    [XmlElement("ZAP")]
    public List<GfExportRecord>? Records { get; set; }
}


/// Заголовок файла GF (для экспорта)

public class GfExportHeader
{
    [XmlElement("VERSION")]
    public string Version { get; set; } = "P5.00";

    [XmlElement("FILE_TYPE")]
    public string FileType { get; set; } = "GF";
...
/// Запись о застрахованном лице (для экспорта)

public class GfExportRecord
{
    ...
    [XmlElement("DN_GIS")]
    public DnGisExportInfo? DnGis { get; set; }


    [XmlElement("DN_LIST")]
    public DnListExportResult? DnList { get; set; }

    [XmlElement("DN_PLAN")]
    public DnPlanExportResult? DnPlan { get; set; }
...
/// Данные ГИС ОМС (для экспорта)

public class DnGisExportInfo
{
...
/// Результат сверки списка ЗЛ на ДН (для экспорта)

public class DnListExportResult
{
...
/// Результат сверки план-графика (для экспорта)

public class DnPlanExportResult
{
